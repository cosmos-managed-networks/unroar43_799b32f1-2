...
# then log output of file /code/bin/unroar43d
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/unroar43d \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/unroar43d | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/unroar43d /usr/bin/unroar43d

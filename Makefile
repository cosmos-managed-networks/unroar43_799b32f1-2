...
HTTPS_GIT := github.com/unroar43/unroar43.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=unroar43 \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=unroar43d \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/unroar43d ./cmd/unroar43d
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/unroar43d.exe ./cmd/unroar43d
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/unroar43d
...
draw-deps:
	...
	@goviz -i ./cmd/unroar43d -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c unroar43 --local -f chains.yaml
endif
...
set-testnet-configs:
	unroar43d config set client chain-id chainid-1
	unroar43d config set client keyring-backend test
	unroar43d config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | unroar43d keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | unroar43d keys add acc1 --recover`
